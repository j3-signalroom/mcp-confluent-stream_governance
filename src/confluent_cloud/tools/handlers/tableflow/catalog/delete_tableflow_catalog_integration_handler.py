...
class DeleteTableflowCatalogIntegrationArguments(BaseModel):
    """Arguments for deleting a Tableflow Catalog Integration"""
...
class DeleteTableFlowCatalogIntegrationHandler(BaseToolHandler):
    """Handler for deleting Tableflow Catalog Integrations in Confluent Cloud"""
    ...
    async def handle(
        ...
        Args:
            ...
            tool_arguments: Arguments containing catalog integration ID
        ...
        if tool_arguments is None:
            return self.create_response(
                "No arguments provided for deleting Tableflow Catalog Integration",
                is_error=True
            )
        
        try:
            # Parse and validate arguments
            args = DeleteTableflowCatalogIntegrationArguments.model_validate(tool_arguments)
        ...
        try:
            ...
            # Note: This API includes query params in the URL path
            url = (
                f"/tableflow/v1/catalog-integrations/{args.id}"
            ...
            if response.status_code >= 400:
                ...
                return self.create_response(
                    f"Failed to delete Tableflow catalog integration {args.id}: {error_text}",
                    is_error=True
                )
            
            result = await response.json()
            return self.create_response(
                f"Tableflow catalog integration {args.id} deleted: {json.dumps(result, indent=2)}"
            )
            
        except Exception as e:
            return self.create_response(
                f"Error deleting Tableflow catalog integration: {str(e)}",
                is_error=True
            )
    ...
    def get_tool_config(self) -> ToolConfig:
        ...
        return ToolConfig(
            ...
            inputSchema=DeleteTableflowCatalogIntegrationArguments.model_json_schema()
        )
