...
class DeleteTableflowTopicArguments(BaseModel):
    """Arguments for deleting a Tableflow topic"""
...
class DeleteTableFlowTopicHandler(BaseToolHandler):
    """Handler for deleting Tableflow topics in Confluent Cloud"""
    ...
    async def handle(
        ...
        if tool_arguments is None:
            return self.create_response(
                "No arguments provided for deleting Tableflow topic",
                is_error=True
            )
        
        try:
            # Parse and validate arguments
            args = DeleteTableflowTopicArguments.model_validate(tool_arguments)
        ...
        try:
            ...
            # Uses display_name as path parameter
            url = (
            ...
            if response.status_code >= 400:
                ...
                return self.create_response(
                    f"Failed to delete Tableflow topic {args.display_name}: {error_text}",
                    is_error=True
                )
            
            result = await response.json()
            return self.create_response(
                f"Tableflow Topic {args.display_name} deleted: {json.dumps(result, indent=2)}"
            )
            
        except Exception as e:
            return self.create_response(
                f"Error deleting Tableflow topic: {str(e)}",
                is_error=True
            )
    ...
    def get_tool_config(self) -> ToolConfig:
        ...
        return ToolConfig(
            ...
            inputSchema=DeleteTableflowTopicArguments.model_json_schema()
        )
