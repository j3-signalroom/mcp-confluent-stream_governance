...
class ListTableFlowTopicArguments(BaseModel):
    """Arguments for listing Tableflow topics"""
...
class ListTableFlowTopicsHandler(BaseToolHandler):
    """Handler for listing Tableflow topics in Confluent Cloud"""
    ...
    async def handle(
        ...
        Returns:
            Result containing the list of Tableflow topics
        """
        try:
            # Parse and validate arguments
            args = ListTableFlowTopicArguments.model_validate(tool_arguments or {})
        ...
        try:
            ...
            url = (
                f"/tableflow/v1/tableflow-topics"
            ...
            if response.status_code >= 400:
                ...
                return self.create_response(
                    f"Failed to list Tableflow topics for {kafka_cluster_id}: {error_text}",
                    is_error=True
                )
            
            result = await response.json()
            return self.create_response(
                f"Tableflow Topics: {json.dumps(result, indent=2)}"
            )
            
        except Exception as e:
            return self.create_response(
                f"Error listing Tableflow topics: {str(e)}",
                is_error=True
            )
    ...
    def get_tool_config(self) -> ToolConfig:
        ...
        return ToolConfig(
            ...
            inputSchema=ListTableFlowTopicArguments.model_json_schema()
        )
